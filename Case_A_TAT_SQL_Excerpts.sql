...
stakeholder confirmation. An external dashboard development team implements
the visual layer from defined consumption requirements. It may aggregate and
filter the authoritative outputs at the specified KPI grain, but it does not
rederive the reporting unit, clock, hold deduction, Net TAT, eligibility, or
...
customer and department; monthly customer overall), and an explicit handoff
block defining how the dashboard implementation consumes the authoritative
outputs without rederiving the reporting unit, clock, hold deduction, Net TAT,
eligibility, or Hit/Miss classification. Operational interpretation is
documented separately in the dashboard guide, which translates source statuses
into user actions without altering the calculation.
*/
