# ERP KPI Definition, Source Logic & BI Scoping

```
Contested ERP metric
        │
        ▼
Confirmed core KPI definition
(frozen calculation rules · implemented in T-SQL)
        │
        ▼
┌────────────────────────────────────┐
│              OUTPUT A              │
│   Authoritative unit foundation    │
│    Project / reconciliation row    │
│   Open · Closed · Container · DQ   │
└───────────┬───────────────┬────────┘
            │               │
      ┌─────▼─────┐    ┌────▼──────┐
      │ OUTPUT C  │    │ OUTPUT B  │
      │ Customer  │    │ Customer  │
      │ × Month   │    │ × Dept    │
      │ × Service │    │ × Month   │
      │   Level   │    │ × Service │
      └─────┬─────┘    │   Level   │
            │          └────┬──────┘
            ▼               ▼
     Customer trend    Department attainment
      (KPI consumer)     (KPI consumer)

Output A also directly supports:
Weekly throughput · Part-number analysis · 52-week customer view

Downstream rule: consume the authoritative outputs; do not re-derive reporting-unit, eligibility, or TAT-result logic.
```

This portfolio shows how I take a vague business request and turn it into a
confirmed, auditable KPI definition, an authoritative data foundation, and
implementation-ready dashboard requirements. The work is drawn from an
aviation MRO ERP analytics engagement; all customer names, contract wording,
schema details, and internal identifiers have been anonymized or paraphrased,
and the examples use synthetic naming throughout.

My role on the engagement spanned business-definition discovery, ERP data
validation, source-logic design in T-SQL, downstream consumption requirements,
visualization review, and the governance needed to keep each metric stable
once agreed. An external development team implements the dashboard UI against
the supplied requirements; I review whether each view answers the confirmed
business question and whether its measures, grain, filters, and interactions
consume the authoritative outputs correctly.

---

## What this portfolio proves

- Defined and validated an authoritative ERP turnaround-time reporting unit
  across standalone, parent, child, detached, and multi-level rework
  structures, with container and data-quality cases handled explicitly.
- Designed a reusable reporting foundation supporting customer, department,
  weekly, part-number, and 52-week KPIs without duplicating the core
  turnaround-time logic across separate implementations.
- Implemented the confirmed rules in T-SQL and used post-freeze reconciliation
  to correct implementation defects without reopening the agreed business
  definition.
- Translated the authoritative data foundation into implementation-ready
  dashboard requirements, including the correct source output, reporting
  period, measures, breakdowns, filters, drill-through behaviour, and
  visualization review for each dependent KPI.

---

## What's here, and the order to read it

**If you have time for one thing, read Case A. The scoping framework below is
the method that produced it.**

1. **Case A — Contractual Turnaround Time (TAT)** — the flagship, and the
   piece to read if you only read one. Its five artifacts show the definition,
   architecture, implementation, governance, visualization handoff, and
   adoption layers:

   - [Case_A_TAT_Case_Study.md](Case_A_TAT_Case_Study.md) — how the KPI
     definition was reasoned out, validated, frozen, and translated into
     downstream implementation requirements. Start here.
   - [Case_A_KPI_Data_Foundation.md](Case_A_KPI_Data_Foundation.md) — how the
     validated reporting-unit logic was extended into an authoritative
     semantic foundation for several dependent KPIs at different business
     grains.
   - [Case_A_TAT_SQL_Excerpts.sql](Case_A_TAT_SQL_Excerpts.sql) — how the
     shared logic was implemented, including structural safeguards and the
     reusable weekly and part-number foundation fields.
   - [Case_A_Decision_Log.xlsx](Case_A_Decision_Log.xlsx) — the decision and
     change trail, including post-freeze defects, business rulings, and how
     each issue was classified.
   - [Case_A_Dashboard_Interpretation_Guide.md](Case_A_Dashboard_Interpretation_Guide.md)
     — the adoption layer being prepared for operational handoff: what each
     status means, how users should interpret it, and what action follows.

2. **KPI Scoping Framework**
   ([KPI_Scope_Matrix.xlsx](KPI_Scope_Matrix.xlsx)) — the method behind the
   cases. One row per requested KPI, carrying each from a synthetic or
   paraphrased request through definition, risk triage, and readiness for
   estimate. Read the *Instructions* sheet first, then skim the matrix. This is
   the lens the rest of the portfolio applies.

3. **Case B — Receiving Activity & Queue** — breadth evidence. The
   [sanitized SQL excerpts](Case_B_Receiving_SQL_Excerpts.sql) and
   [decision log](Case_B_Decision_Log.xlsx) show the same method applied to a
   different problem shape: one request that unfolded into four distinct
   workflows.

4. **Supporting governance evidence** — across the artifacts above, the
   decision logs and Case A's operational-precondition material show how data
   quality, implementation changes, and definition stability are managed after
   the core rule is agreed.

---

## Numbering

Portfolio identifiers are internal to this portfolio and do not correspond to
any engagement's numbering.

- **Case A** and **Case B** are the two featured portfolio cases.
- **PF-01 … PF-15** identify the rows in the scoping framework (a
  representative excerpt of the wider scoping work). Case A is PF-01;
  Case B is PF-05.

---

## Framework origin

No BI scoping template was provided to me. After taking ownership of the
discovery and KPI-definition process, I found there was no structured way to
carry a request from "what the customer said" through to "what we can build
and defend," so I designed one: the field set, the definition-risk versus
data-readiness triage, the decision-tracking convention, and the
delivery-readiness status vocabulary. The version here is a simplified,
anonymized adaptation of the working framework. It is my own methodology
asset, not an internal company document.

---

## The through-line

One principle runs through every piece: **make predictable downstream errors
difficult or impossible by construction, rather than relying on documentation
alone.**

Where a downstream mistake could happen, the design closes it off in the data
itself. Judged values and displayed values cannot disagree; structurally
ambiguous and contradictory source-evidence rows carry null metrics, while
authoritative statuses and inclusion controls keep non-eligible rows out of
the affected performance measures. Other uncertainty is surfaced and either
handled under a documented conservative rule or excluded from the affected
measure rather than guessed.

The same discipline extends past the query: each definition states the
operational preconditions it depends on and a governance loop for changes, so
data-quality risks are addressed upstream wherever possible rather than
discovered only after dashboard delivery.

---

## A note on what these show

- **Judgment and validation discipline** — the case studies center on
  decisions where the obvious reading was wrong, and on catching those
  through assumption-testing against real data before a definition was
  frozen.
- **Definition governance** — freezing a metric, distinguishing a business
  definition from its implementation, and correcting implementation defects
  as controlled fixes without reopening the agreed definition. Case A shows
  four categories of defect found by reconciling output after the freeze, and
  two questions that were referred back to the client instead, because they
  were matters of contract scope rather than implementation.
- **Source-to-dashboard handoff** — defining not only the authoritative
  datasets and aggregation rules, but also how each KPI should consume them:
  the appropriate grain, measures, filters, drill-through path, and visual
  treatment needed to answer the confirmed business question.
- **End-to-end value realization** — a correct query is not yet a delivered
  metric. Case A closes four gaps between definition and operational value:
  source-data practices before go-live, implementation fidelity through
  post-freeze reconciliation, business-question fidelity through
  visualization review, and user adoption through a status-to-action
  interpretation guide.

*Contents are anonymized. Identifiers, thresholds, contract language, and
schema details have been replaced with synthetic equivalents; the reasoning
and structure are unchanged.*
