...
| **Part-number duration** | What is average Net TAT by project part number and department, weekly and monthly? | Output A using unit-level Net TAT and project part number | Supports average Net TAT and completed-job count without redefining the TAT calculation; median is deferred from v1. |
...
Each dependent KPI may require its own approved aggregation, ranking, time
window, and visual treatment, based on its business question. Those consumption
requirements are specified and reviewed separately; the implementation layer
may not redefine the unit, clock, population, hold treatment, or result.

### From data foundation to dashboard specification

The shared foundation removed the need to redesign the calculation for every
view, but it did not remove the need for KPI-specific visualization decisions.
For implementation handoff, I translated each business question into a compact
consumer specification covering:

- the source output to consume;
- the grain of that output;
- the reporting time grain;
- the primary breakdown;
- filter and drill dimensions; and
- the measure or result the view must communicate.

I then reviewed proposed views against those specifications. This prevented a
technically functioning visual from substituting a different analytical
question — for example, encoding a percentage pattern where the requested KPI
was weekly completed volume split into Hit and Miss. The external development
team retained responsibility for UI implementation and visual styling; the
review protected the connection between the confirmed business question, the
authoritative data, and what the user would actually see.
...
The selected architecture therefore separates two kinds of responsibility:

- each dependent KPI receives its own approved grain, aggregation, ranking,
  time-window, and presentation requirements;
- the implementation layer may not redefine the reporting unit, clock, hold
  treatment, population, or result semantics.
...
- a clear boundary between authoritative source logic, KPI-specific
  visualization requirements, and dashboard implementation;
