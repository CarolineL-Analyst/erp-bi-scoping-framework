...
It identifies the reporting unit, applies the confirmed population and endpoint
rules, calculates hold-adjusted Net TAT, classifies the result, and preserves
the fields needed for reconciliation and drill-through.
...
The distinction is structural. Container and data-quality rows remain visible,
but their metric fields and inclusion flags prevent them from entering
attainment rates, averages, medians, or rankings.
...
Both are derived directly from the valid unit-level population in Output A.
...
| **Population** | Apply the confirmed contractual population once at the source layer. Downstream views do not recreate hidden customer or project-status filters. |
...
| **Containers and DQ** | Keep visible for reconciliation and correction; exclude from normal rates, averages, medians, and rankings by construction. |
...
| **Empty periods** | No qualifying closed units means no performance population / NULL, not an artificial 0% result. |
...
Once the authoritative reporting-unit layer existed, the dependent Phase 1
KPIs did not require separate turnaround-time calculations.
...
- **Closed Week Start Date** — a single authoritative week key derived from the
  qualifying completion date.
...
| **Weekly attribution** | Use one authoritative closed-week key based on final TAT completion date. | Keeps weekly KPIs aligned across counts, rankings, averages, and 52-week views. |
...
| **Treat Outputs B and C as separate sources of truth, or average department percentages into a customer rate.** | **Rejected** | Both summaries must be recomputed from valid Output A unit rows. Pre-aggregated percentages are not additive and cannot safely be averaged. |
