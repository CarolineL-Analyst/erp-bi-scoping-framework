...
*A KPI definition, source-logic, and visualization-handoff case study.
...
I led the analytical design and source-logic delivery for a KPI dashboard
engagement with an aviation MRO operator running on its MRO ERP platform. The
engagement covered a broad, cross-functional KPI portfolio spanning logistics,
inventory, finance, commercial, project management, and operations. Across
these, I owned the work from business definition through source logic and
downstream visualization
requirements — running discovery sessions with the client, validating data
readiness and process behaviour inside the ERP, deciding v1 scope versus
future enhancement, preparing the authoritative SQL outputs, and reviewing
whether proposed and implemented views correctly answered each KPI's business
question. An external development team implements the dashboard UI against
those requirements; it does not independently define or re-derive the KPI
logic.
...
dashboard implementation boundary is correspondingly clear: consume the
authoritative status and metric fields, aggregate them only according to the
specified KPI grain, and do not introduce a visual-layer correction or
fallback that is absent from the source logic.
...
I also reviewed the proposed dashboard views against their stated business
questions. Where a visual encoded the wrong analytical emphasis — for example,
showing a colour pattern when the KPI required weekly Hit and Miss volumes — I
specified the required measures, breakdowns, filters, and interaction
behaviour and directed the implementation team toward a more appropriate
visual treatment. The development team retained implementation responsibility;
the review keeps the resulting view aligned with the confirmed KPI rather than
merely displaying available fields.

Together, the work closes four distinct gaps: source-data practice before
go-live, implementation fidelity through post-freeze reconciliation,
business-question fidelity through visualization review, and user adoption
through status-to-action guidance. The hidden rigor in the source logic becomes
visible to users when, for example, **Shipped but completion date missing** does
not appear as ordinary overdue work, but as a clear instruction to correct the
completion evidence rather than infer an endpoint.

---


## 6. Key alternatives considered and why they were not selected

The Decision Log preserves the full decision trail. The table below is a
reader-facing summary of the alternatives that most directly shaped the final
KPI design.

| Design question | Alternative considered | Decision | Why it was not selected |
|---|---|---|---|
| **What closes the TAT clock?** | Use Project Closed status or the project-complete date. | **Rejected** | Project closure is an administrative state, not the confirmed contractual endpoint. It can also lose the time precision available on packing-list completion. |
| **What counts as one reporting unit?** | Measure one result at the root or family level. | **Rejected** | Independently returned child deliverables would be collapsed into one family result, distorting both workload and the Hit/Miss denominator. |
| **How should multiple packing lists be handled?** | Automatically select the first or latest completion date. | **Rejected** | Either choice would invent a rule for ambiguous evidence. Historical exceptions included different document purposes and mixed dispatch patterns, not a reliable second completion event. |
| **How should holds be scoped?** | Pool parent and child holds across the family before deduction. | **Rejected** | Holds are recorded against the project whose clock they govern. Pooling them would deduct time from reporting units to which the hold did not apply. |
| **How should multiple active receipts be handled?** | Automatically choose the earliest active receipt as the start. | **Rejected** | A plausible-looking start date is not the same as an authoritative one. Non-unique receipt evidence is surfaced as data quality until the source is resolved. |
| **What should happen when dispatch exists but completion evidence is missing?** | Infer the endpoint from dispatch evidence or project status. | **Rejected** | This would fabricate an unverified completion timestamp and hide a source-data contradiction inside an operational-looking result. |
| **How should deeper project structures be handled?** | Hard-code the known hierarchy depth or add a special case for a third level. | **Rejected** | A depth-specific rule would fail on the next unseen structure. Reporting-unit generation is based on qualifying dispatch evidence instead. |
| **How should cancelled projects be treated?** | Automatically exclude every cancelled project from the KPI population. | **Escalated to business decision** | Profiling showed that cancellation did not necessarily end the return process, and some cancelled projects still reached the confirmed outbound endpoint. Excluding them technically would silently decide whether the contractual obligation ends at cancellation—a population rule that belongs to the client. |

These choices were not all technically impossible. They were rejected because
they would move an unresolved business assumption into code, create a
plausible-but-unverified result, or make downstream inconsistency easier.
...
dashboard development team can implement without re-deriving it — with the
business question, consumption rules, and visualization behaviour kept
